...
## Update (Jun 2026 — Setoran Kas: Export Excel + Filter Tanggal)
- FILTER TANGGAL di tab Setoran Kas: input Dari/Sampai + preset "Hari ini" / "Hari Acara (13 Sep)" / "Semua tanggal ✕" (data-testid `setoran-daterange`, `setoran-date-from/to`, `setoran-preset-today/event/all`). Backend `GET /api/admin/cash-settlement?from=&to=` filter by created_at WIB (YYYY-MM-DD). Rekap `_collect_cash_settlement(from,to)` di-refactor jadi helper bersama.
- EXPORT EXCEL: tombol `setoran-export-btn` → `GET /api/admin/cash-settlement/export?from=&to=` (superadmin) → xlsx 3 sheet: (1) Setoran per Petugas (order/tiket/total cash/sudah disetor/belum + baris TOTAL), (2) Rincian per Tanggal (petugas·tanggal·order·tiket·cash), (3) Riwayat Setoran/Closing. Ikut filter tanggal aktif.
- RECEIVE ikut filter: `POST /admin/cash-settlement/receive {seller, from?, to?}` — bila filter tanggal aktif, hanya order cash dalam rentang itu yang ditandai disetor (konsisten dgn tampilan). Model CashReceive + Field alias from/to (populate_by_name).
- DIUJI e2e (curl + screenshot, Jun 2026): filter hari-ini benar (Admin 1 cash 80rb/3 tiket), filter tanggal masa depan → kosong; export xlsx valid (PK, 3 sheet, angka benar); receive dgn filter today → 80rb, outstanding→0; UI tampil tombol Export + filter + preset. Data uji ZZTEST + settlement dibersihkan total.
- Perlu REDEPLOY agar naik ke kbikepri.com.
